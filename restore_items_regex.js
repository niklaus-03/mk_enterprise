...
const startRegex = /\{selectedSuppliers\.length > 0 && \(\s*<div style=\{\{ background: '#fffbeb'/;
const endRegex = /<div style=\{\{ position: 'relative' \}\}>\s*<label style=\{\{ display: 'block', fontSize: 13, fontWeight: 700, color: '#475569', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0\.5px' \}\}>Notes<\/label>/;
...
const newJSX = `
                ...
                {deliveryForm.suppliers_data && deliveryForm.suppliers_data.length > 0 ? (
                  deliveryForm.suppliers_data.map((supplierObj, sIdx) => (
                    <div key={sIdx} style={{ background: '#f8fafc', padding: '20px', borderRadius: 16, border: '1px solid #e2e8f0', marginBottom: 16 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                        {supplierObj.items && supplierObj.items.map((item, idx) => (
                  ...
                  <div style={{ background: '#f8fafc', padding: '20px', borderRadius: 16, border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 120, marginBottom: 20 }}>
...
code = code.slice(0, startIndex) + newJSX + "\n" + code.slice(endIndex);

fs.writeFileSync(filePath, code);
console.log('Successfully restored supplier-specific items mapping using regex matching!');
