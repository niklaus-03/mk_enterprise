...
import { Bell, Heart, Package, FileText, AlertTriangle, Clock, CheckCircle2, MoreHorizontal, Truck } from 'lucide-react';
...
export default function NotificationDropdown({ user, className, style, bellSize = 22, iconColor = 'inherit', customButton }) {
  ...
  const getIcon = (type) => {
    switch (type) {
      case 'trip_started': return { icon: <Truck size={16} color="#fff" />, bg: 'linear-gradient(135deg, #10b981, #059669)' };
      case 'trip_bypass_request': return { icon: <Truck size={16} color="#fff" />, bg: 'linear-gradient(135deg, #f59e0b, #d97706)' };
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0, lineHeight: 1.4 }}>
                      <span style={{ fontSize: 14, color: 'var(--text)' }}>
                        ...
                        {n.type === 'trip_started' ? (
                          <div style={{ marginTop: 4, color: 'var(--text-muted)', fontSize: 13, display: 'flex', flexDirection: 'column', gap: 6 }}>
                            <div>Started trip to {n.metadata?.destination}. Vehicle: {n.metadata?.vehicle_number}</div>
                            {n.metadata?.items && n.metadata.items.length > 0 && (
                              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 4 }}>
                                {n.metadata.items.map((item, idx) => (
                                  <div key={idx} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 6, padding: '2px 8px', fontSize: 11, fontWeight: 600, color: 'var(--primary)' }}>
                                    {item.quantity}x {item.product_name}
                                  </div>
                                ))}
                              </div>
                            )}
                            {n.metadata?.total_value > 0 && <div style={{ fontWeight: 700, color: '#10b981' }}>Total Load Value: ₹{n.metadata.total_value.toLocaleString('en-IN')}</div>}
                          </div>
                        ) : (
