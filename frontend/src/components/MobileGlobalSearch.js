...
export default function MobileGlobalSearch({ customButton, desktop }) {
  ...
  const [recentSearches, setRecentSearches] = useState(() => {
    try {
      const stored = localStorage.getItem(`mk_recent_searches_${user?.id || 'guest'}`);
  ...
  const handleRecentAdd = (item) => {
    setRecentSearches(prev => {
      ...
      localStorage.setItem(`mk_recent_searches_${user?.id || 'guest'}`, JSON.stringify(updated));
      return updated;
    });
  };
    ...
    const delayDebounce = setTimeout(async () => {
      ...
      try {
        const [custRes, prodRes, invRes, supRes, mgrRes, drvRes] = await Promise.all([
          ...
          isWalkinManager ? Promise.resolve([]) : supplierApi.getAll(query).catch(() => []),
          isWalkinManager ? Promise.resolve({ managers: [] }) : managerApi.getAll().catch(() => ({ managers: [] })),
          isWalkinManager ? Promise.resolve({ drivers: [] }) : driverApi.getAll().catch(() => ({ drivers: [] }))
        ]);
