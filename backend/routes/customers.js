...
router.post('/', async (req, res) => {
  try {
    ...
    const isOverride = ['supervisor', 'admin'].includes(req.user.role) && req.body.override_creator_id;
    const customerData = {
      name: name.trim(),
      phone: phone || '',
      ...
      gstin: gstin || '',
      created_by: isOverride ? req.body.override_creator_id : req.user.id,
      added_by_admin: !!isOverride,
    };
