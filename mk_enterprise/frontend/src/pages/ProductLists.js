  ...
  return (
    <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div style={{ fontSize: 14, color: '#64748b' }}>Create product groupings and share them with managers</div>
      ...
      {showForm && (
        <div className="card" style={{ marginBottom: 20, border: '1.5px solid #6366f1', boxShadow: '0 10px 25px -5px rgba(99, 102, 241, 0.15)' }}>
          <div className="card-header" style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'linear-gradient(135deg, #f8fafc, #f1f5f9)', borderBottom: '1.5px solid #e2e8f0' }}>
            <div style={{ background: editId ? '#fef3c7' : '#ecfdf5', color: editId ? '#d97706' : '#059669', borderRadius: '50%', width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                {filteredProducts.map(p => (
                  <label key={p._id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderBottom: '1px solid #f1f5f9', cursor: 'pointer', margin: 0 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                      ...
                      <span style={{ fontSize: 11, color: '#64748b' }}>₹{p.price} / {p.unit}</span>
                    </div>
                  </label>
                ))}
        ...
        <div className="card-body no-pad">
          {loading ? (
            ...
            <div className="empty-state" style={{ padding: '40px 24px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 600, color: '#475569' }}>No lists found</div>
            </div>
          ) : (
                    ...
                    return (
                      <tr key={list._id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                        <td style={{ padding: '10px 14px', fontWeight: 600, color: '#0f172a' }}>
                          {list.name}
                          {!isOwner && <span style={{ fontSize: 10, background: '#e0f2fe', color: '#0369a1', padding: '2px 6px', borderRadius: 4, marginLeft: 8 }}>{isAdmin ? 'Created by' : 'Shared by'} {list.created_by?.display_name || list.created_by?.username}</span>}
                        </td>
                        <td style={{ padding: '10px 14px', color: '#64748b' }}>
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                            {displayProducts.slice(0, 5).map(p => (
                              <span key={p._id} style={{ background: '#f1f5f9', padding: '2px 6px', borderRadius: 4, fontSize: 12 }}>{p.name}</span>
                            ))}
                            {displayProducts.length > 5 && <span style={{ background: '#e2e8f0', padding: '2px 6px', borderRadius: 4, fontSize: 12 }}>+{displayProducts.length - 5} more</span>}
                          </div>
                        </td>
                        {isAdmin && (
                        <td style={{ padding: '10px 14px' }}>
                          {list.shares?.length > 0 ? (
                            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                              {list.shares.map(s => (
                                <span key={s._id || s.manager_id?._id} style={{ fontSize: 12, background: '#fef3c7', color: '#d97706', padding: '2px 6px', borderRadius: 4 }}>
            ...
            <div className="modal-body" style={{ padding: '20px' }}>
              ...
              <form onSubmit={handleShareSubmit}>
                ...
                {selectedManagerId && (
                  <div className="form-group">
                    <label style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ fontSize: 12, fontWeight: 'normal', color: '#64748b' }}>Checked = Shared, Unchecked = Hidden</div>
                    </label>
                    <div style={{ maxHeight: 300, overflowY: 'auto', border: '1px solid #e2e8f0', borderRadius: 8, padding: 12, display: 'grid', gap: 8 }}>
                      {shareModal.products.map(p => {
                        ...
                        return (
                          <label key={p._id} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '6px', borderRadius: 6, background: isShared ? '#f0fdf4' : '#fef2f2', border: `1px solid ${isShared ? '#bbf7d0' : '#fecaca'}` }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 24 }}>
                  <button type="button" className="btn btn-outline" onClick={() => setShareModal(null)}>Cancel</button>
        ...
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: 400 }}>
            <div style={{ display: 'flex', gap: 16 }}>
              <div style={{ background: '#fee2e2', color: '#ef4444', padding: 12, borderRadius: '50%' }}><AlertTriangle size={24} /></div>
              <div>
                <h3 style={{ margin: 0 }}>Delete List?</h3>
                <p style={{ margin: '8px 0', color: '#64748b' }}>This action cannot be undone.</p>
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 24 }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5 }}>
                  <thead style={{ background: '#f8fafc', borderBottom: '1.5px solid #e2e8f0' }}>
                    <tr>
                      ...
                      <th style={{ padding: '10px 14px', textAlign: 'right', fontWeight: 700, color: 'var(--text-muted)' }}>Stock</th>
                    </tr>
                  </thead>
                  <tbody>
