...
import { dashboardApi, settlementApi, orderApi, deliveryApi, supplierApi, customerApi, productApi, productListApi } from '../utils/api';
                        ...
                        return (
                          <tr key={`${c._id}-${idx}`} style={{ borderBottom: '1px solid #f3f4f6', background: paid ? '#f0fdf4' : idx % 2 === 0 ? '#fff' : '#fafafa' }}>
                            <td style={{ padding: '10px 14px' }}>
                              {c.invoice_number ? (
                                paid ? (
                                  <span style={{ color: 'var(--text-muted)', fontWeight: 700, fontFamily: 'monospace', fontSize: 12.5, textDecoration: 'line-through', opacity: 0.7 }}>
                                    {c.invoice_number}
                                  </span>
                                ) : (
                            ...
                            <td style={{ padding: '10px 14px' }}>
                              {paid ? (
                                ...
                                <button className="btn btn-success btn-sm"
                                  onClick={() => {
                                    setPayModal({
                                      invoice_id: c.invoice_id || (isHistoricalView ? c._id : (c.type === 'walkin' ? c._id : null)),
                                      customer_id: c.customer_id || (isHistoricalView ? null : (c.type === 'registered' ? c._id : null)),
                                      name: c.name,
                                      balance: c.balance,
